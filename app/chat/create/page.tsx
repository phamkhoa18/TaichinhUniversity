...
/* ═══════════════════════════════════════
   Pipeline step icon mapping
   ═══════════════════════════════════════ */
const STEP_GROUPS: Array<{ keywords: string[]; icon: React.ComponentType<any>; color: string }> = [
  { keywords: ['kiểm tra', 'guard', 'an toàn'], icon: Shield, color: '#10b981' },
  { keywords: ['phân tích', 'ngữ cảnh', 'intent', 'rewrite', 'standalone'], icon: Brain, color: '#3b82f6' },
  { keywords: ['rag', 'vector', 'truy vấn', 'search', 'tìm', 'embedding', 'retriev'], icon: Database, color: '#8b5cf6' },
];

function mapStepsToIcons(steps: string[]) {
  const seen = new Set<number>();
  const result: Array<{ icon: React.ComponentType<any>; color: string }> = [];
  for (const step of steps) {
    const lower = step.toLowerCase();
    for (let g = 0; g < STEP_GROUPS.length; g++) {
      if (seen.has(g)) continue;
      if (STEP_GROUPS[g].keywords.some(kw => lower.includes(kw))) {
        seen.add(g);
        result.push({ icon: STEP_GROUPS[g].icon, color: STEP_GROUPS[g].color });
        break;
      }
    }
  }
...
/* ═══════════════════════════════════════
   ...
   ═══════════════════════════════════════ */
function TypingDots({ steps }: { steps?: string[] }) {
  const icons = steps ? mapStepsToIcons(steps) : [];

  return (
    <div className="flex items-center gap-2.5 py-1">
      {/* Condensed icon pills */}
      {icons.length > 0 && (
      ...
      {/* Bouncing dots — matching Chatbot widget style */}
    ...
    >
      ...
      {isBot ? (
        <div className="flex items-center gap-1.5 mb-1 ml-1">
          ...
          <span className="text-[12px] font-medium text-gray-500">UFM Bot</span>
        </div>
      ) : (
      ...
      <div className={`flex flex-col gap-1.5 ${isBot ? 'max-w-[90%] md:max-w-[85%]' : 'max-w-[85%] md:max-w-[75%]'}`}>
        ...
        >
          {message.isTyping ? (
            <TypingDots steps={message.pipelineSteps} />
          ) : isBot ? (
            message.isStreaming ? (
...
export default function ChatCreatePage() {
  ...
  const sendMessage = useCallback(async (text: string) => {
    ...
    try {
      const res = await fetch(`${FASTAPI_CHAT_URL}/api/v1/chat/stream-tokens`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          query: text.trim(),
          ...
          try {
            ...
            if (event.type === 'node') {
              ...
              const finalMsg: ChatMessage = {
                id: botMsgId,
                role: 'bot',
                content: fullResponse || 'Xin lỗi, mình không thể xử lý yêu cầu này.',
                timestamp: new Date(),
