    ...
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500 pb-16">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-[22px] font-bold tracking-tight text-slate-800">AI Chatbot — VectorDB</h1>
            <div className="flex items-center gap-2 mt-0.5">
              <p className="text-[12px] text-slate-500 font-medium">Quản lý dữ liệu RAG Pipeline (PostgreSQL + pgvector)</p>
              <span className={`inline-flex items-center gap-1 text-[10px] font-bold px-1.5 py-0.5 rounded-full ${
                dbConnected ? 'bg-emerald-50 text-emerald-600' : 'bg-red-50 text-red-500'
              }`}>
                {dbConnected ? <><Wifi size={9}/> Connected</> : <><WifiOff size={9}/> Offline</>}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
        {[
          ...
          { label: 'Vector đã nhúng', value: hasEmbeddings, icon: Zap, color: 'text-amber-600', bg: 'bg-amber-50' },
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 overflow-hidden shadow-sm">
        <div className="flex items-center border-b border-slate-100 px-2 lg:px-4 bg-white">
          {[
            { key: 'vectordb', label: 'VectorDB Documents', icon: Database },
                ...
                <tbody className="divide-y divide-slate-100">
                  {loading && vectorDocs.length === 0 ? (
                    ...
                    <tr><td colSpan={7} className="text-center py-16">
                      ...
                      <p className="text-[13px] font-bold text-slate-500">Chưa có dữ liệu trong VectorDB</p>
        ...
        {activeTab === 'upload' && (
          <div className="p-5 md:p-8 space-y-6">
            <div onDragOver={e => e.preventDefault()} onDrop={handleDrop}
              ...
              <p className="text-[12px] font-medium text-slate-500 mt-1">File sẽ được chunk → embed → nạp vào VectorDB (pgvector)</p>
            </div>
            ...
            <div className="flex justify-center mt-5">
              <button onClick={handleUpload} disabled={isUploading || uploadFiles.length === 0}
                ...
                {isUploading ? <><Loader2 size={16} className="animate-spin" /> Đang xử lý...</> : <><Upload size={16} /> Bắt đầu nạp VectorDB</>}
              </button>
            </div>
          </div>
        )}
          ...
          <div className="p-5 md:p-8 space-y-5">
            ...
            <div className="flex items-center gap-3 pb-4 border-b border-slate-100">
              ...
              <div>
                ...
                <p className="text-[11px] text-slate-500 font-medium">Soạn thảo bằng trình soạn thảo → Gemini chuyển sang Markdown → Nạp VectorDB</p>
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2">
              ...
              >
                {isComposing ? (
                  ...
                  <><Sparkles size={16} /> Chuyển đổi & Nạp VectorDB</>
                )}
              </button>
            </div>
          </div>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm" onClick={() => setShowDeleteConfirm(null)}>
          <div onClick={e => e.stopPropagation()} className="bg-white rounded-2xl shadow-2xl p-6 max-w-sm mx-4 w-full animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-[16px] font-bold text-slate-800 text-center">Xóa tài liệu khỏi VectorDB?</h3>
            <p className="text-[12px] text-slate-500 text-center mt-2 font-medium">
              File <span className="font-bold text-slate-700">{showDeleteConfirm}</span> sẽ bị soft-delete khỏi PostgreSQL VectorDB.
            </p>
            ...
            <div className="p-6 flex-1 overflow-auto bg-[#f8fafc]">
              ...
              {editLoading ? (
                <div className="flex flex-col items-center justify-center h-[300px] text-slate-400 gap-3">
                  ...
                  <p className="text-[13px] font-medium">Đang lấy nội dung từ VectorDB...</p>
                </div>
              ) : (
                <textarea
                  value={editContent}
