...
import {
  ...
  ListTree, ToggleLeft, ToggleRight, ChevronDown, GripVertical, User
} from 'lucide-react'
...
export default function EditNewsPage() {
  ...
  const [newsMeta, setNewsMeta] = useState<{ authorName?: string, updatedAt?: string, createdAt?: string } | null>(null)
      ...
      .then(json => {
        if (json.success && json.data) {
          ...
          setFormData({
            ...
            hideThumbnail: data.hideThumbnail || false,
            hidePdfPreview: data.hidePdfPreview || false,
          });
          setNewsMeta({ 
            authorName: data.author?.name || 'VĐTSĐH',
            updatedAt: data.updatedAt,
            createdAt: data.createdAt,
          });
          setThumbnail(data.thumbnail || null);
          setAttachedFile(data.attachedFile || null);
          setTocEnabled(data.tocEnabled || false);
          if (data.publishedAt) setDate(new Date(data.publishedAt));
        } else {
          showToast.error('Không tìm thấy bài viết');
          router.push('/admin/thong-bao');
        }
      })
      .catch(() => showToast.error('Lỗi khi tải dữ liệu bài viết'))
  ...
  if (isInitialLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[400px]">
        ...
        <p className="text-sm font-medium text-slate-500 mt-4">Đang tải dữ liệu bài viết...</p>
      </div>
    )
  }
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 mb-6">
        ...
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="text-[20px] font-bold tracking-tight text-slate-800 truncate">Sửa bài viết</h1>
            <div className="flex items-center gap-3 mt-1.5 flex-wrap">
              {currentStatus && (
                <span className="inline-flex items-center gap-1.5 text-[10px] font-bold text-slate-700 bg-slate-100 px-2.5 py-0.5 rounded-full uppercase tracking-wide">
              ...
              {newsMeta && (
                <>
                  <span className="text-[12px] text-slate-500 flex items-center gap-1.5 font-medium border-l border-slate-200 pl-3">
                    <User className="w-3.5 h-3.5 text-slate-400" />
                    <strong>Đăng bởi:</strong> {newsMeta.authorName}
                  </span>
                  <span className="text-[12px] text-slate-500 flex items-center gap-1.5 font-medium border-l border-slate-200 pl-3">
                    <CalendarIcon className="w-3.5 h-3.5 text-slate-400" />
                    <strong>Tạo:</strong> {newsMeta.createdAt ? format(new Date(newsMeta.createdAt), 'dd/MM/yyyy HH:mm') : ''}
                  </span>
                  <span className="text-[12px] text-slate-500 flex items-center gap-1.5 font-medium border-l border-slate-200 pl-3">
                    <Clock className="w-3.5 h-3.5 text-slate-400" />
                    <strong>Cập nhật:</strong> {newsMeta.updatedAt ? format(new Date(newsMeta.updatedAt), 'dd/MM/yyyy HH:mm') : ''}
                  </span>
                </>
              )}
            </div>
