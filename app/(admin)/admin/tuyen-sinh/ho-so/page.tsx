...
import {
  Search, Users, Clock, CheckCircle2, XCircle, Eye, Filter, Loader2,
  ClipboardList, GraduationCap, ShieldCheck, Ban, Download, CalendarDays
} from 'lucide-react'
...
export default function HoSoListPage() {
  ...
  const fetchData = useCallback(async (page = 1) => {
    ...
    if (roundFilter !== 'all') params.set('roundId', roundFilter)
    if (search.trim()) params.set('search', search.trim())

    const res = await fetch(`/api/admin/admissions/applicants?${params}`)
    ...
    setLoading(false)
  }, [statusFilter, roundFilter, search])
  ...
  const handleQuickStatus = async (id: string, newStatus: string) => {
    setUpdatingId(id)
    try {
      await fetch(`/api/admin/admissions/applicants/${id}`, {
        method: 'PUT',
  ...
  const stats = [
    ...
    { label: 'Đủ điều kiện', value: counts.QUALIFIED + counts.ADMITTED, icon: CheckCircle2, color: 'text-emerald-600', bg: 'bg-emerald-50/50', border: 'border-emerald-200/80' },
    { label: 'Từ chối / Không đủ', value: counts.NOT_QUALIFIED + counts.REJECTED, icon: XCircle, color: 'text-rose-600', bg: 'bg-rose-50/50', border: 'border-rose-200/80' },
  ]
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row items-baseline justify-between gap-4">
        <div>
          <h1 className="text-[22px] font-bold tracking-tight text-slate-800">Quản lý Hồ sơ Ứng viên</h1>
          <p className="text-[15px] text-slate-500 font-medium mt-1">Xem và xét duyệt toàn bộ hồ sơ tuyển sinh Thạc sĩ, Tiến sĩ.</p>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-3 items-center justify-between">
        <div className="flex-1 w-full lg:max-w-sm relative group">
          ...
          <input type="text" placeholder="Tìm theo tên, SĐT, CCCD, email..."
            value={search} onChange={(e) => setSearch(e.target.value)}
            className="w-full pl-9 pr-4 h-10 bg-white border border-slate-200 rounded-xl text-[14px] font-medium text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-[#005496]/50 focus:border-[#005496] transition-all" />
        </div>
        <div className="flex items-center gap-2">
          <Select value={roundFilter} onValueChange={setRoundFilter}>
            <SelectTrigger className="h-10 w-[220px] rounded-xl border-slate-200 text-[13px] font-medium bg-white">
              <CalendarDays className="w-3.5 h-3.5 mr-1.5 text-slate-400" />
              <SelectValue placeholder="Đợt tuyển sinh" />
            </SelectTrigger>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
        {loading ? (
          <div className="flex items-center justify-center p-16"><Loader2 className="w-6 h-6 animate-spin text-[#005496]" /></div>
        ) : applicants.length === 0 ? (
          <div className="flex flex-col items-center justify-center p-16 text-center">
            <div className="w-16 h-16 bg-slate-50 border border-slate-100 text-slate-300 rounded-full flex items-center justify-center mb-4"><Users className="w-8 h-8" /></div>
            <h3 className="text-[15px] font-bold text-slate-800">Chưa có hồ sơ nào</h3>
            <p className="text-slate-500 max-w-sm mt-1.5 text-[13px] font-medium">Chưa có ứng viên nào nộp hồ sơ tuyển sinh.</p>
          </div>
        ) : (
          <>
            <Table className="w-full text-slate-600">
              <TableHeader className="bg-slate-50/80 border-b border-slate-200">
                <TableRow className="hover:bg-transparent border-none">
                  ...
                  <TableHead className="h-10 py-2 px-5 text-[13px] font-semibold text-slate-500">Đợt tuyển sinh</TableHead>
                ...
                {applicants.map((app, idx) => {
                  ...
                  return (
                    <TableRow key={app._id} className="hover:bg-slate-50/60 transition-colors group border-b border-slate-100">
                      ...
                      <TableCell className="py-3.5 px-5">
                        {roundInfo && typeof roundInfo === 'object' ? (
                          <div className="flex flex-col gap-0.5">
                            <Link href={`/admin/tuyen-sinh/dot/${roundInfo._id}`} className="text-[13px] font-semibold text-slate-700 hover:text-[#005496] transition-colors truncate max-w-[180px]">
                              {roundInfo.name}
                            </Link>
                            <Badge variant="outline" className={`text-[10px] px-1.5 py-0 w-fit border uppercase tracking-wider font-bold ${roundInfo.level === 'TIEN_SI' ? 'text-amber-600 bg-amber-50/50 border-amber-200' : 'text-slate-500 bg-slate-50/50 border-slate-200'}`}>
                              {roundInfo.level === 'TIEN_SI' ? 'Tiến sĩ' : 'Thạc sĩ'}
                            </Badge>
                          </div>
                        ) : (
                      ...
                      <TableCell className="py-3.5 px-5 text-right">
                        <div className="flex items-center justify-end gap-1 opacity-40 group-hover:opacity-100 transition-opacity">
                          ...
                          {app.status === 'PENDING' && (
                            <>
                              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-lg text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 bg-transparent" title="Duyệt"
                                disabled={updatingId === app._id} onClick={() => handleQuickStatus(app._id, 'QUALIFIED')}>
                                {updatingId === app._id ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
                              </Button>
                              <Button variant="ghost" size="sm" className="h-8 w-8 p-0 rounded-lg text-slate-400 hover:text-rose-600 hover:bg-rose-50 bg-transparent" title="Từ chối"
                                disabled={updatingId === app._id} onClick={() => handleQuickStatus(app._id, 'REJECTED')}>
            ...
            <div className="flex items-center justify-between px-5 py-3 border-t border-slate-100 bg-slate-50/50">
              ...
              <div className="flex gap-1.5">
                <Button variant="outline" size="sm" className="h-8 px-3 rounded-lg text-[13px] font-medium border-slate-200 shadow-none bg-white"
                  disabled={pagination.page <= 1} onClick={() => fetchData(pagination.page - 1)}>Trước</Button>
                <span className="flex items-center px-3 text-[13px] font-bold text-slate-600">{pagination.page} / {pagination.totalPages}</span>
                <Button variant="outline" size="sm" className="h-8 px-3 rounded-lg text-[13px] font-medium border-slate-200 shadow-none bg-white"
                  disabled={pagination.page >= pagination.totalPages} onClick={() => fetchData(pagination.page + 1)}>Sau</Button>
              </div>
            </div>
          </>
        )}
